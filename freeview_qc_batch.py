...
For each subject, opens:
    ...
    $MODEL_DIR/<session>/<subject>.nii.gz                (top layer, colormap=lut, outline=1)
...
def main():
    ...
    parser.add_argument("--model-dir", type=Path, required=True,
                         help="Root of label volumes, e.g. /.../model (expects <model-dir>/<session>/<subject>.nii.gz)")
    parser.add_argument("--session", required=True,
                         help="Session label used verbatim: ses-<session> under --bids-dir, "
                              "and <session> under --model-dir")
    ...
    for subject in subjects:
        ...
        label_path = model_dir / args.session / f"{subject}.nii.gz"
